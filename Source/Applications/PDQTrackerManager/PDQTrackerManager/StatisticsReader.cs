...
namespace PDQTrackerManager
{
    ...
    public class StatisticsReader : IDisposable
    {
        ...
        /// </summary>
        /// <param name="severity">The index that defines which statistic in the given category is to be retrieved.</param>
        /// <returns>A lookup table containing a collection of data from the historian for each metadata record found with the given signal defined.</returns>
        /// <remarks>
        /// All parameters--<see cref="ArchiveFilePath"/>, <see cref="StartTime"/>, and <see cref="EndTime"/>--must
        /// be set manually before attempting to read statistics from the archive.
        /// </remarks>
        public Dictionary<MetadataRecord, IEnumerable<IDataPoint>> Read(int severity)
        {
            //string signalReferenceEnding = string.Format("{0}!-ST", severity);

            string signalReferencePattern = string.Format("^{0}!.+!PT-ST[0-9]+$", severity);

            return m_metadataRecords
                .Where(record => Regex.IsMatch(record.Synonym1, signalReferencePattern))
                .ToDictionary(record => record, record => m_archiveReader.ReadData(record.HistorianID, StartTime, EndTime));
        }
