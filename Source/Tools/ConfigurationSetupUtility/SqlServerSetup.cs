...
namespace ConfigurationSetupUtility
{
    ...
    public class SqlServerSetup
    {
        ...
        public void GrantDatabaseAccess(string loginName)
        {
            ...
            ExecuteStatement("IF DATABASE_PRINCIPAL_ID('PDQTrackerAdminRole') IS NULL CREATE ROLE [PDQTrackerAdminRole] AUTHORIZATION [dbo]");
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('{0}') IS NOT NULL AND DATABASE_PRINCIPAL_ID('PDQTrackerAdminRole') IS NOT NULL EXEC sp_addrolemember N'PDQTrackerAdminRole', N'{0}'", loginName));
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('PDQTrackerAdminRole') IS NOT NULL EXEC sp_addrolemember N'db_datareader', N'PDQTrackerAdminRole'"));
            ExecuteStatement(string.Format("IF DATABASE_PRINCIPAL_ID('PDQTrackerAdminRole') IS NOT NULL EXEC sp_addrolemember N'db_datawriter', N'PDQTrackerAdminRole'"));
        }
                    ...
                    while ((object)line != null)
                    {
                        ...
                        else
                        {
                            if (trimLine.StartsWith("CREATE DATABASE") || trimLine.StartsWith("ALTER DATABASE") || trimLine.StartsWith("USE"))
                                line = line.Replace("PDQTracker", DatabaseName);
