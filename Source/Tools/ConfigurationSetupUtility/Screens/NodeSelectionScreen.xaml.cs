...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class NodeSelectionScreen : UserControl, IScreen
    {
        ...
        private void UpdateDataGrid()
        {
            ...
            {
                m_infoTextBlock.Text = "Please select the node you would like the PDQTracker to use.";
            }
            else
            {
            ...
            if (applyChangesToService)
                nodeId = GetNodeIdFromConfigFile("PDQTracker.exe.config");
            else if (applyChangesToLocalManager)
                nodeId = GetNodeIdFromConfigFile("PDQTrackerManager.exe.config");
            else
                nodeId = Guid.Empty;
            ...
            if (applyChangesToService)
                TryAddNodeInfo("PDQTracker.exe.config");

            if (applyChangesToLocalManager)
                TryAddNodeInfo("PDQTrackerManager.exe.config");
        ...
        private IDbConnection GetConnectionFromConfigFile()
        {
            ...
            string configFileName = Directory.GetCurrentDirectory() + "\\PDQTracker.exe.config";
        ...
        {
            return GetValueOfSystemSetting("PDQTracker.exe.config", "CompanyName");
        }
        ...
        // Occurs when the user selects a different node to be used by the PDQTracker.
