...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class ApplyConfigurationChangesScreen : UserControl, IScreen
    {
        ...
        private void InitializeState()
        {
            ...
            m_PDQTrackerManagerLocalCheckBox.IsEnabled = managerOptionsEnabled;
            m_PDQTrackerManagerWebCheckBox.IsEnabled = webManagerOptionEnabled;

            // If the options are disabled, they must also be unchecked.
            if (!managerOptionsEnabled)
                m_PDQTrackerManagerLocalCheckBox.IsChecked = false;
            ...
            m_state["applyChangesToLocalManager"] = m_PDQTrackerManagerLocalCheckBox.IsChecked.Value;
            m_state["applyChangesToWebManager"] = m_PDQTrackerManagerWebCheckBox.IsChecked.Value;
        ...
        // Occurs when the user chooses to apply changes to the PDQTracker service.
        private void PDQTrackerServiceCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToService"] = true;
        }

        // Occurs when the user chooses to not apply changes to the PDQTracker service.
        private void PDQTrackerServiceCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToService"] = false;
        }

        // Occurs when the user chooses to changes to the local PDQTracker Manager application.
        private void PDQTrackerManagerLocalCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToLocalManager"] = true;
        }

        // Occurs when the user chooses to not apply changes to the local PDQTracker Manager application.
        private void PDQTrackerManagerLocalCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToLocalManager"] = false;
        }

        // Occurs when the user chooses to apply changes to the PDQTracker Manager web application.
        private void PDQTrackerManagerWebCheckBox_Checked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
                m_state["applyChangesToWebManager"] = true;
        }

        // Occurs when the user chooses to not apply changes to the PDQTracker Manager web application.
        private void PDQTrackerManagerWebCheckBox_Unchecked(object sender, RoutedEventArgs e)
        {
            if (m_state != null)
