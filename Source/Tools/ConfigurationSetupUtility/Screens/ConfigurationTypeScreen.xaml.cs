...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class ConfigurationTypeScreen : UserControl, IScreen
    {
        ...
        public ConfigurationTypeScreen()
        {
            ...
            try
            {
                // Determine if the PDQTracker is installed in this folder (could be stand-alone debug application or service), we check exe
                // instead of config file since config file can remain between installations. Since only the PDQTracker can be setup with a
                // XML or web service configuration, we disable these radio buttons for PDQTracker Manager only installations.
                string exeFileName = Directory.GetCurrentDirectory() + "\\PDQTracker.exe";

                if (!File.Exists(exeFileName))
                {
                    // If the PDQTracker is not be installed user may have chosen to only install the PDQTracker Manager on this system,
            ...
            {
                // Not failing if we cannot determine if PDQTracker is available...
            }
        }
