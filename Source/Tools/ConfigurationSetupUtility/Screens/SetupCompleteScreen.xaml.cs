...
namespace ConfigurationSetupUtility.Screens
{
    ...
    public partial class SetupCompleteScreen : UserControl, IScreen
    {
        ...
        private ServiceController m_PDQTrackerServiceController;
        ...
        {
            InitializeComponent();
            InitializePDQTrackerServiceController();
            ...
            get
            {
                ...
                if (m_state != null)
                {
                    try
                    {
                        ...
                        try
                        {
                            ConfigurationFile PDQTrackerConfig = ConfigurationFile.Open("PDQTracker.exe.config");

                            // Some of the crypto settings elements were renamed for consistency, remove the old ones
                            CategorizedSettingsElementCollection cryptoSection = PDQTrackerConfig.Settings["cryptographyServices"];
                            cryptoSection.Remove("RetryDelayInterval");
                            cryptoSection.Remove("MaximumRetryAttempts");

                            // Example connection settings are updated between builds - remove the section and PDQTracker will add back the latest
                            PDQTrackerConfig.Settings.Remove("exampleConnectionSettings");

                            // Data publisher categories are now always lower case (such that code references are case insensitive)
                            PDQTrackerConfig.Settings.Remove("dataPublisher");

                            PDQTrackerConfig.Save(ConfigurationSaveMode.Full);
                        }
                        catch
                        {
                        ...
                        // If the user requested it, start or restart the PDQTracker service
                        if (m_serviceStartCheckBox.IsChecked.Value)
                        {
                            try
                            {
#if DEBUG
                                Process.Start("PDQTracker.exe");
#else
                                m_PDQTrackerServiceController.Start();
#endif
                            }
                            catch
                            {
                                MessageBox.Show("The configuration utility was unable to start PDQTracker service, you will need to manually start the service.", "Cannot Start Windows Service", MessageBoxButton.OK, MessageBoxImage.Information);
                            }
                        }

                        // If the user requested it, start the PDQTracker Manager.
                        if (m_managerStartCheckBox.IsChecked.Value)
                        {
                            Process.Start("PDQTrackerManager.exe");
                        }
                    }
                    finally
                    {
                        if (m_PDQTrackerServiceController != null)
                            m_PDQTrackerServiceController.Close();
                    }
                }
        ...
        public Dictionary<string, object> State
        {
            ...
            set
            {
                ...
                if (Convert.ToBoolean(m_state["restarting"]))
                    m_serviceStartCheckBox.Content = "Restart the PDQTracker";
            }
        }
        ...
        // Initializes the PDQTracker service controller.
        private void InitializePDQTrackerServiceController()
        {
            ServiceController[] services = ServiceController.GetServices();
            m_PDQTrackerServiceController = services.SingleOrDefault(svc => string.Compare(svc.ServiceName, "PDQTracker", true) == 0);
        }

        // Initializes the state of the PDQTracker service checkbox.
        private void InitializeServiceCheckboxState()
        {
#if DEBUG
            bool serviceInstalled = File.Exists("PDQTracker.exe");
#else
            bool serviceInstalled = m_PDQTrackerServiceController != null;
#endif
            m_serviceStartCheckBox.IsChecked = serviceInstalled;
            m_serviceStartCheckBox.IsEnabled = serviceInstalled;
        }

        // Initializes the state of the PDQTracker Manager checkbox.
        private void InitializeManagerCheckboxState()
        {
            bool managerInstalled = File.Exists("PDQTrackerManager.exe");
        ...
        {
            string configFileName = Path.Combine(Directory.GetCurrentDirectory(), "PDQTracker.exe.config");
            ...
            {
                string archiveBinding = string.Format(GrafanaArchiveBinding, "stat", "http.rest://localhost:6360/api/grafana");
            ...
            if (setupHistorian)
            {
                ...
                {
                    string archiveBinding = string.Format(GrafanaArchiveBinding, historianAcronym, "http.rest://localhost:6460/api/grafana");
        ...
        private void ValidateNodeSettings()
        {
            ...
            try
            {
                ...
                if (m_state.TryGetValue("selectedNodeId", out selectedNodeId) && !string.IsNullOrWhiteSpace(selectedNodeId.ToNonNullString()))
                {
                    ...
                    {
                        nodeSettings.Add("RemoteStatusServerConnectionString", "server=localhost:8520;integratedSecurity=true");
                    }
                    else
                    {
                    ...
                    if (!nodeSettings.TryGetValue("AlarmServiceUrl", out alarmServiceUrl))
                        nodeSettings.Add("AlarmServiceUrl", "http://localhost:5022/alarmservices");
