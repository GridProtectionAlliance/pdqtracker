...
namespace Setup
{
    public partial class Main : Form
    {
        ...
        private void buttonInstall_Click(object sender, EventArgs e)
        {
            ...
            // Verify that .NET 4.5.2 is installed
            try
            {
                RegistryKey net46 = Registry.LocalMachine.OpenSubKey("SOFTWARE\\Microsoft\\.NETFramework\\v4.0.30319\\SKUs\\.NETFramework,Version=v4.5.2");

                if (net46 == null)
                {
                    if (MessageBox.Show("Microsoft .NET 4.5.2 does not appear to be installed on this computer. The .NET 4.5.2 framework is required to be installed before you continue installation. Would you like to install it now?", ".NET 4.5.2 Check", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        Process net45Install;
                        const string netInstallPath = "Installers\\NDP46-KB2901907-x86-x64-AllOS-ENU.exe";

                        if (File.Exists(netInstallPath))
                        {
                            try
                            {
                                // Attempt to launch .NET 4.5.2 installer...
                            ...
                            {
                                // At a minimum open folder containing .NET 4.5.2 installer since its available to run...
                                net45Install = new Process();
                        ...
                        {
                            net45Install = new Process();
                            net45Install.StartInfo.FileName = "https://www.microsoft.com/en-us/download/details.aspx?id=42643";
                            net45Install.StartInfo.UseShellExecute = true;
                            net45Install.Start();
                        }
                    }
                    else
                        runSetup = (MessageBox.Show("Would you like to attempt installation anyway?", ".NET 4.5.2 Check", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes);
                }
                else
                    runSetup = true;
            }
            ...
            {
                runSetup = (MessageBox.Show("The setup program was not able to determine if Microsoft .NET 4.5.2 is installed on this computer. The .NET 4.5.2 framework is required to be installed before you continue installation. Would you like to attempt installation anyway?", ".NET 4.5.2 Check", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes);
            }
